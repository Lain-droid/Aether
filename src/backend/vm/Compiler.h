...
namespace AetherVisor {
    namespace VM {
        ...
        public:
            ...
            void* CompileToNative(const std::vector<uint8_t>& bytecode);
            void ExecuteNative(void* native_code);
            void FreeNativeCode(void* native_code);
            
        private:
            void* m_code_buffer;
            size_t m_buffer_size;
            
            void EmitX86Instruction(uint8_t* buffer, size_t& offset, const uint8_t* instruction, size_t length);
            void TranslateBytecodeToX86(const std::vector<uint8_t>& bytecode, uint8_t* native_buffer, size_t& size);
        };
        ...
        private:
            static void EliminateDeadCode(std::vector<uint8_t>& bytecode);
            static void OptimizeJumps(std::vector<uint8_t>& bytecode);
            static void MergeInstructions(std::vector<uint8_t>& bytecode);
            static void OptimizeConstants(std::vector<uint8_t>& bytecode);
        };
